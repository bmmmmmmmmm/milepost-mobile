...
function connect(props: Props) {
  const wrap = (WrappedComponent: React.FunctionComponent) => {
    const ConnectComponent = (comprops: any) => {
      ...
      let stateProps = props.mapStateToProps ? props.mapStateToProps(state) : {}
      let dispatchProps = props.mapDispatchToProps ? props.mapDispatchToProps(dispatch) : {}
      let allProps = { ...comprops }

      const updateProps = () => {
        stateProps = props.mapStateToProps ? props.mapStateToProps(state) : {}
        dispatchProps = props.mapDispatchToProps ? props.mapDispatchToProps(dispatch) : {}
